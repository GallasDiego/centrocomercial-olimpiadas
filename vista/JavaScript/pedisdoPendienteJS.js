...
forms = new Array("formGastos","formComision","formPedidosPendientes");//Array con los id de todos los formularios
var formulario = "";
...
function obtenerID(){
  ...
  for(a=0; a<=formulario.elements.length-1; a++){
    if(formulario.elements[a].type=='hidden'){
      ...
      if(formulario.elements[a].id=='proveedor'){
...
function retornarDatos(){
    obtenerID();
    var proveedor=selects();
    var cad='';
    var fechaDesde=document.getElementById('sqlFechaDesde1').value;
    var fechaHasta=document.getElementById('sqlFechaHasta1').value;
    cad='proveedor='+encodeURIComponent(proveedor)+'&fechaDesde1='+encodeURIComponent(fechaDesde)+'&fechaHasta1='+encodeURIComponent(fechaHasta);
  return cad;
}
    ...
    if(select.selectedIndex != undefined && select.selectedIndex != -1){//Recorro la longitud del select
      for(j=0; j < select.options.length; j++){ 
        if(select.options[j].selected == true){//Se buscan solo los seleccionados
          proveedor += escape(select.options[j].value)+';';//Acumulo en un string cada valor separado por punto y coma
        }
      }
    }
  return proveedor;
...
function enviarFormulario() 
{
  ...
  conexion.onreadystatechange = function(){
    if (conexion.readyState==4){
      document.getElementById("tablaPedidosPendientes").innerHTML=conexion.responseText;
    }
    else{ 
  ...
  conexion.open('POST','../../../../negocio/PedidosPendientes.php', true);
